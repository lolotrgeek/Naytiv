...
export class ListDetailComponent implements OnInit {
 ...
 ngOnInit() {
   ...
   this.sub = this.route.params.subscribe((params: any) => {
      this.id = params['id'];
      this.firebaseService.getMyList(this.id).subscribe((list) => {
        this.ngZone.run(() => {
          for (let prop in list) {
            ...
            if (prop === "description") {
              this.description = list[prop];
            }
            if (prop === "imagepath") {
              this.imagepath = list[prop];
            }                       
          }
        });
      });
  ...
  if(this.image){
    ...
    this.firebaseService.uploadFile(this.imagePath).then((uploadedFile: any) => {
          ...
          this.firebaseService.getDownloadUrl(this.uploadedImageName).then((downloadUrl: string) => {
            this.firebaseService.editList(id,this.description,downloadUrl).then((result:any) => {
              alert(result)
            }, (error: any) => {
                alert(error);
            });
